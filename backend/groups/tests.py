...
class GroupAPITests(TestCase):
    def setUp(self):
        ...
        self.user3 = User.objects.create_user(email="charlie@example.com", password="password123")
    ...
    def test_create_group(self):
        ...
        response = self.client.post(
            "/api/groups/",
            json.dumps({"name": "Dinner Club", "group_type": "casual", "privacy": "invite-only"}),
    ...
    def test_edit_group_as_leader(self):
        ...
        GroupMembership.objects.create(user=self.user1, group=group, role=GroupMembership.Role.LEADER)
    ...
    def test_edit_group_as_member_fails(self):
        group = Group.objects.create(name="Breakfast Club", created_by=self.user1)
        GroupMembership.objects.create(user=self.user1, group=group, role=GroupMembership.Role.LEADER)
        GroupMembership.objects.create(user=self.user2, group=group, role=GroupMembership.Role.MEMBER)
    ...
    def test_invite_member(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(GroupMembership.objects.filter(user=self.user2, group=group).exists())
